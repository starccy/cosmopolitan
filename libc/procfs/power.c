...
typedef int32_t(__msabi *CallNtPowerF)(int, void *, uint32_t, void *, uint32_t);
...
static void batt_design(struct pfs_batt *out) {
  ...
  if (!have) {
    static const struct NtGuid guid = {
        // GUID_DEVINTERFACE_BATTERY
        0x72631e54,
        0x78A4,
        0x11d0,
    ...
    if (!lsz || !lst)
      return;
    if (lsz(&need, &guid, 0, 0 /* present only */) || need < 2 || need > 1024)
      return;
    if (lst(&guid, 0, paths, need, 0))
      return;
    ...
    if (DeviceIoControl(h, 0x294040 /* IOCTL_BATTERY_QUERY_TAG */, &wait, 4,
                        &tag, 4, &got, 0)) {
      struct {
        uint32_t Tag, Level;
        int32_t AtRate;
      ...
      if (DeviceIoControl(h, 0x294044 /* ..._QUERY_INFORMATION */, &q, sizeof q,
                          &bi, sizeof bi, &got, 0) &&
...
bool pfs_battery(struct pfs_batt *out) {
  ...
  if (f(5 /* SystemBatteryState */, 0, 0, &b, sizeof b))
    return false;
...
int pfs_cpu_topology(uint8_t *coreid, uint8_t *pkgid, int cap) {
  ...
  static GlpiExF f;
  if (!f)
    f = (GlpiExF)pfs_sym(u"kernel32.dll", "GetLogicalProcessorInformationEx");
  if (!f)
    return 0;
  static char buf[65536];  // guarded by the carrier's lock
  uint32_t len = sizeof buf;
  if (!f(0xffff /* RelationAll */, buf, &len))
    return 0;
  if (cap > 64)
    cap = 64;
    ...
    for (uint32_t off = 0; off + 32 <= len;) {
      ...
      if (r == rel) {
        ...
        for (int g = 0; g < groups; g++) {
          ...
          if (*(uint16_t *)(ga + 8) != 0)
            continue;  // group 0 only
          uint64_t mask = *(uint64_t *)ga;
          for (int c = 0; c < cap; c++) {
            if (!(mask >> c & 1))
              continue;
            if (pass)
              pkgid[c] = (uint8_t)npkg;
            else
              coreid[c] = (uint8_t)ncore;
            if (c + 1 > n)
              n = c + 1;
          }
        }
