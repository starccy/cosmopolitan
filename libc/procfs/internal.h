#ifndef COSMOPOLITAN_LIBC_PROCFS_INTERNAL_H_
#define COSMOPOLITAN_LIBC_PROCFS_INTERNAL_H_
...
// The /proc emulation, in two halves. The generators (pid.c, net.c,
// sysinfo.c, sysctl.c, sysfs.c and the XNU counterparts in xnu.c) read the
// host and emit Linux-formatted text into a pfs_buf, knowing nothing about
// descriptors. The carrier (open.c, stat.c, readlink.c, handle.c, list.c,
// cache.c, path.c) decides when a generator runs and serves the result
// behind a kFdProc descriptor. Everything is in memory: nothing is ever
// written to disk, and the tree exists only for the process that looks.

// The hosts served. Everywhere else the real /proc, or its absence, is
// left alone.
#define PC_HOSTED() (IsWindows() || IsXnuSilicon())

#define PC_PATH_MAX 256  // a virtual path
#define PID_DIR_MS  200  // one process's volatile files
#define NET_MS      250  // the interface counters
...
void pfs_printf(struct pfs_buf *b, const char *fmt, ...) printfesque(2);
...
// GetProcAddress with the DLL cached, for the handful of APIs that live in
// DLLs cosmo does not import (loading those at every process start would
// cost more than the emulation is used).
...
// The volatile four move while a process runs and come out of one
// OpenProcess pass; the stable rest never changes for a process's lifetime.
// starttime identifies the incarnation, so a recycled pid is recognized as
// a new process.
...
uint64_t pfs_net_inode(uint8_t proto, uint8_t family, uint32_t pid,
                       uint16_t lport, uint16_t rport, const uint8_t laddr[16],
                       const uint8_t raddr[16]);

// ---------------------------------------------------------------------------
// fd.c. Our own process's full descriptor table, read out of the fd table.
...
// NT only; the XNU side answers nothing here.
...
// files out, the way the kernel does. NT only.
...
// path.c. The path model. `sub` is what follows "/proc": "" or "/...".
// Parsing resolves the self alias, so every node names the real pid.
enum kind {
  K_OTHER,  // unknown: mapped, found absent
  K_ROOT,
  K_TOP,      // /proc/<name>
  K_PID_DIR,  // /proc/<pid>
...
bool pc_is_proc(const char *path);  // "/proc" or "/proc/..."
bool pc_is_sys(const char *path);   // "/sys" or "/sys/..."
bool pc_spelled_self(const char *sub);
const char *pc_link_name(const struct node *n);  // exe, cwd, root or 0
unsigned pc_link_mode(const char *vpath);        // 0 unless vpath is a symlink
...
// The virtual path a caller's spelling names: absolute, or relative to a
// kFdProc directory. 1 with the normalized path in out, 2 when the path
// resolved out of the tree (a real path in out, for the host), 0 when it
// is not ours, -1 with errno.
int pc_resolve(int dirfd, const char *path, char *out, size_t outsz);
// The link a path goes through when its name is exe, cwd or root of a
// process followed by more: 1 with the real path in out, 0 otherwise, -1
// with errno.
int pc_through_link(const struct node *n, const char *vpath, char *out,
                    size_t outsz);

// cache.c. The one lock every generation runs under, the re-entrancy
// guard, and the per-process cache of the volatile files.
...
// list.c. Entries of a directory of the tree: count, -1 when vpath is not
// a directory here, -2 when it is one but absent (a dead pid). The caller
// frees the entries.
int pc_list_dir(const char *vpath, struct pfs_virtent **out);

// sysfs.c. The emulated slices of /sys. kind: -1 absent, 0 a file, 1 a
// directory.
...
// handle.c. What sits behind a kFdProc descriptor.
struct ProcfsHandle {
  atomic_int refs;
  pthread_mutex_t lock;
  char *p;  // the text, or a link's target
  size_t n;
  size_t pos;
  int64_t gen;  // slot generation the content came from, 0 if not a slot
  bool dir;
  bool link;  // opened with O_PATH|O_NOFOLLOW: names the link itself
  struct pfs_virtent *ents;
  int nents;
  char vpath[PC_PATH_MAX];
};
...
COSMOPOLITAN_C_END_
#endif /* COSMOPOLITAN_LIBC_PROCFS_INTERNAL_H_ */
