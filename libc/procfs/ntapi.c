...
static struct pfs_proc *next_proc(void) {
  if (g_nprocs == g_capprocs) {
    ...
    if (!grown)
      return 0;
    if (g_nprocs)
      memcpy(grown, g_procs, g_nprocs * sizeof *grown);
    g_procs = grown;
    g_capprocs = cap;
  }
...
static bool snap_query_locked(void) {
  ...
  for (int tries = 0;; tries++) {
    ...
    uint32_t need = 0;
    NtStatus st =
        NtQuerySystemInformation(kNtSystemProcessInformation, buf, cap, &need);
    if (!st)
      break;
    free(buf);
    buf = 0;
...
static void snap_refresh_locked(void) {
  ...
  if (g_snap_ms && t - g_snap_ms < PFS_SNAP_MS)
    return;
  ...
  int64_t snap =
      CreateToolhelp32Snapshot(kNtTh32csSnapprocess | kNtTh32csSnapthread, 0);
  if (snap == -1)
    return;
  ...
  e.dwSize = sizeof e;
  for (bool32 ok = Process32First(snap, &e); ok; ok = Process32Next(snap, &e)) {
    struct pfs_proc *p = next_proc();
    if (!p)
      break;
    memset(p, 0, sizeof *p);
...
int pfs_threads_of(uint32_t pid, uint64_t *out, int cap) {
  if (IsXnuSilicon())
    return pfs_xnu_threads(pid, out, cap);
  pthread_mutex_lock(&g_snap_lock);
  snap_refresh_locked();
  int n = 0;
  for (int i = 0; i < g_nthreads && n < cap; i++)
    if (g_threads[i].pid == pid)
      out[n++] = g_threads[i].tid;
  pthread_mutex_unlock(&g_snap_lock);
  return n;
}
...
uint64_t pfs_boot_filetime(void) {
  ...
  if (!boot)
    boot = pfs_now_filetime() - GetTickCount64() * 10000;
  return boot;
}
