...
typedef uint32_t(__msabi *GetExtTableF)(void *, uint32_t *, int32_t, uint32_t,
                                        uint32_t, uint32_t);

const char *const pfs_net_files[] = {"tcp", "tcp6", "udp", "udp6",
                                     "dev", "unix", 0};
...
static uint8_t tcp_state_to_linux(uint32_t nt) {
  switch (nt) {
    case 2:
      return 0x0a;  // LISTEN
    case 3:
      return 0x02;  // SYN_SENT
    case 4:
      return 0x03;  // SYN_RCVD
    case 5:
      return 0x01;  // ESTAB
    case 6:
      return 0x04;  // FIN_WAIT1
    case 7:
      return 0x05;  // FIN_WAIT2
    case 8:
      return 0x08;  // CLOSE_WAIT
    case 9:
      return 0x0b;  // CLOSING
    case 10:
      return 0x09;  // LAST_ACK
    case 11:
      return 0x06;  // TIME_WAIT
    default:
      return 0x07;  // CLOSED, DELETE_TCB, anything new
  }
}
...
static void add_row(uint8_t proto, uint8_t family, uint8_t state, uint32_t pid,
                    const void *laddr, uint16_t lport, const void *raddr,
                    uint16_t rport) {
  ...
  if (raddr)
    memcpy(r->raddr, raddr, family == 4 ? 4 : 16);
  r->inode =
      pfs_net_inode(proto, family, pid, lport, rport, r->laddr, r->raddr);
  g_nrows++;
}
...
struct nt_tcp4 {
  uint32_t state, laddr, lport, raddr, rport, pid;
};
struct nt_tcp6 {
  uint8_t laddr[16];
  uint32_t lscope, lport;
  uint8_t raddr[16];
  uint32_t rscope, rport, state, pid;
};
struct nt_udp4 {
  uint32_t laddr, lport, pid;
};
struct nt_udp6 {
  uint8_t laddr[16];
  uint32_t lscope, lport, pid;
};
...
static void net_refresh_locked(void) {
  ...
  if (g_net_ms && t - g_net_ms < NET_REFRESH_MS)
    return;
  ...
  if ((b = fetch(tcp, NT_AF_INET, TCP_TABLE_OWNER_PID_ALL))) {
    ...
    for (uint32_t i = 0; i < n; i++)
      add_row(0, 4, tcp_state_to_linux(r[i].state), r[i].pid, &r[i].laddr,
              nt_port(r[i].lport), &r[i].raddr, nt_port(r[i].rport));
    free(b);
  }
  if ((b = fetch(tcp, NT_AF_INET6, TCP_TABLE_OWNER_PID_ALL))) {
    ...
    for (uint32_t i = 0; i < n; i++)
      add_row(0, 6, tcp_state_to_linux(r[i].state), r[i].pid, r[i].laddr,
              nt_port(r[i].lport), r[i].raddr, nt_port(r[i].rport));
    free(b);
  }
  if ((b = fetch(udp, NT_AF_INET, UDP_TABLE_OWNER_PID))) {
    ...
    for (uint32_t i = 0; i < n; i++)
      add_row(1, 4, 0x07, r[i].pid, &r[i].laddr, nt_port(r[i].lport), zero, 0);
    free(b);
  }
  if ((b = fetch(udp, NT_AF_INET6, UDP_TABLE_OWNER_PID))) {
    ...
    for (uint32_t i = 0; i < n; i++)
      add_row(1, 6, 0x07, r[i].pid, r[i].laddr, nt_port(r[i].lport), zero, 0);
    free(b);
  }
  g_net_ms = pfs_now_ms();
}
...
bool pfs_gen_net_file(struct pfs_buf *b, const char *name) {
  if (IsXnuSilicon())
    return pfs_xnu_gen_net_file(b, name);
  uint8_t proto, family;
  if (!strcmp(name, "dev"))
    return gen_net_dev(b);
  ...
  if (!strcmp(name, "tcp"))
    proto = 0, family = 4;
  else if (!strcmp(name, "tcp6"))
    proto = 0, family = 6;
  else if (!strcmp(name, "udp"))
    proto = 1, family = 4;
  else if (!strcmp(name, "udp6"))
    proto = 1, family = 6;
  else
    return false;
...
int pfs_net_fds_of(uint32_t pid, uint64_t *inodes, int cap) {
  if (IsXnuSilicon())
    return 0;  // no socket table source wired up yet
  pthread_mutex_lock(&g_net_lock);
  net_refresh_locked();
  int n = 0;
  for (int i = 0; i < g_nrows && n < cap; i++)
    if (g_rows[i].pid == pid)
      inodes[n++] = g_rows[i].inode;
  pthread_mutex_unlock(&g_net_lock);
  return n;
}
...
static const char *if_row_of(const char *tab, uint32_t index) {
  ...
  for (uint32_t i = 0; i < n; i++) {
    ...
    if (*(const uint32_t *)(row + IF_ROW2_INDEX) == index)
      return row;
  }
  return 0;
}
...
int pfs_net_ifstats(struct pfs_ifstat *out, int cap) {
  ...
  for (struct NtIpAdapterAddresses *p = aa; p && n < cap; p = p->Next) {
    ...
    s->maclen = p->PhysicalAddressLength > 8 ? 8 : p->PhysicalAddressLength;
...
static bool gen_net_dev(struct pfs_buf *b) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    pfs_printf(b,
               ...
               (unsigned long long)s->rx_pkts, (unsigned long long)s->rx_errs,
               (unsigned long long)s->rx_drop, (unsigned long long)s->tx_bytes,
               (unsigned long long)s->tx_pkts, (unsigned long long)s->tx_errs,
