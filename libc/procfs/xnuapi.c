...
static struct {
  ...
  int (*region_recurse)(uint32_t, uint64_t *, uint64_t *, uint32_t *, void *,
                        uint32_t *);
...
static bool init_locked(void) {
  ...
  if (!IsXnuSilicon() || !__syslib || __syslib->__version < 10)
    return false;
...
bool pfs_xnu_vmstat(struct xnu_vm_statistics64 *out) {
  if (!pfs_xnu_ready() || !g.host_stats64 || !g.host_self)
    return false;
...
int pfs_xnu_cpu_ticks(struct xnu_cpu_ticks *out, int cap) {
  if (!pfs_xnu_ready() || !g.host_cpu_info || !g.host_self)
    return 0;
  uint32_t ncpu = 0, cnt = 0;
  int *info = 0;
  if (g.host_cpu_info(g.host_self(), XNU_PROCESSOR_CPU_LOAD_INFO, &ncpu, &info,
                      &cnt) != 0 ||
      !info)
    return 0;
  int n = (int)ncpu;
...
bool pfs_xnu_fdsock(uint32_t pid, int fd, struct xnu_sock_id *out) {
  ...
  if (r < (int)sizeof u.si)
    return false;
  const struct xnu_socket_fdinfo *s = &u.si;
  if (s->psi.soi_kind != XNU_SOCKINFO_IN && s->psi.soi_kind != XNU_SOCKINFO_TCP)
    return false;
...
long pfs_xnu_ape_program(uint32_t pid, char *buf, size_t n) {
  if (!g.ok)
    return -1;  // set once at init; a stale read only declines
...
int pfs_xnu_listfds(uint32_t pid, struct xnu_proc_fdinfo *out, int cap) {
  if (!pfs_xnu_ready())
    return 0;
  int r =
      g.pidinfo((int)pid, XNU_PROC_PIDLISTFDS, 0, out, cap * (int)sizeof *out);
  return r > 0 ? r / (int)sizeof *out : 0;
}
...
long pfs_xnu_fdpath(uint32_t pid, int fd, char *buf, size_t n) {
  ...
  int r =
      g.pidfdinfo((int)pid, fd, XNU_PROC_PIDFDVNODEPATHINFO, &vi, sizeof vi);
  if (r != (int)sizeof vi)
    return -1;
...
bool pfs_xnu_iflist2(char **out, size_t *len) {
  ...
  for (int tries = 0; tries < 3; tries++) {
    size_t n = 0;
    if (__syslib->__sysctl(mib, 6, 0, &n, 0, 0) != 0 || !n)
      return false;
...
bool pfs_xnu_region(uint64_t *addr, uint64_t *size, uint32_t *depth,
  ...
  if (!pfs_xnu_ready() || !g.region_recurse || !g.task_self)
    return false;
...
static void set_comm(struct pfs_proc *p, const char *name) {
  ...
  p->comm[j] = 0;
  if (!j)
    snprintf(p->comm, sizeof p->comm, "%u", p->pid);
}

static void snap_locked(void) {
  int64_t t = pfs_now_ms();
  if (g_snap_ms && t - g_snap_ms < XNU_SNAP_MS)
    return;
  if (!init_locked())
    return;
